...
class ProductListView(ListView):
    ...
    def get_queryset(self):
        ...
        return super().get_queryset().order_by(f'{"" if ascending == '1' else "-"}{order}')
    ...
    def get_context_data(self, *args, **kwargs):
        ...
        if self.request.user.is_authenticated:
            cart = Cart.objects.filter(user=self.request.user).select_related('product')
...
class ProductUpdateView(LoginRequiredMixin, UpdateView):
    model = Product
    fields = ('name', 'description', 'price', 'quantity', 'category', 'picture', 'available')
    success_url = '/catalog/my_products/'
    template_name = 'create_product.html'
...
class ProductUserListView(LoginRequiredMixin, ListView):
    ...
    def get_queryset(self):
        ascending = self.request.GET.get('asc', '1')
        order = self.request.GET.get('order', 'name')
        return super().get_queryset().filter(owner=self.request.user).order_by(f'{"" if ascending == '1' else "-"}{order}')
